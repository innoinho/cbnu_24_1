...
label_less_stock_threshold = tk.Label(root, text="특정 재고 수량 미만인 품목을 확인하려면 숫자를 입력하세요:")
label_less_stock_threshold.pack()
entry_less_stock_threshold = tk.Entry(root)
entry_less_stock_threshold.pack()

show_less_stock_button = tk.Button(root, text="특정 재고 미만 품목 확인", command=show_less_stock)
show_less_stock_button.pack()

# 새 품목 추가를 위한 입력 상자 및 버튼 추가
label_new_item = tk.Label(root, text="새 품목 추가:")
label_new_item.pack()
entry_new_item = tk.Entry(root)
entry_new_item.pack()

add_item_button = tk.Button(root, text="추가", command=add_item)
