...
search_entry.bind("<KeyRelease>", filter_items)  # 검색어가 입력될 때마다 filter_items 함수 호출

# 품목 선택을 위한 콤보박스 생성
items = list(previous_total_stock.keys())
item_var = tk.StringVar(root)
item_var.set(items[0])  # 초기 선택값을 설정합니다.
item_combobox = ttk.Combobox(frame, textvariable=item_var, state="readonly", justify="center", width=20, height=20)  # width와 height 속성 변경
item_combobox['values'] = items
item_combobox.pack(pady=15, fill=tk.X)

# 콤보박스 값이 변경될 때마다 update_stock 함수 호출
