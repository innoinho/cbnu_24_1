...
def update_stock(event):
    selected_item = item_combobox.get()  # 콤보박스에서 선택된 품목 가져오기
    previous_item_stock = previous_total_stock[selected_item]
...
def add_item():
    ...
    if new_item:
        ...
        item_combobox['values'] = list(previous_total_stock.keys())
        entry_new_item.delete(0, tk.END)

def filter_items(event):
    search_term = search_entry.get().lower()  # 검색어를 소문자로 변환하여 저장
    filtered_items = [item for item in previous_total_stock.keys() if search_term in item.lower()]  # 검색어가 포함된 품목 찾기
    item_combobox['values'] = filtered_items  # 콤보박스의 값들을 필터된 품목으로 설정

def show_all_stock():
    all_stock_window = tk.Toplevel(root)
    all_stock_window.title("전체 재고 확인")
    all_stock_window.geometry("400x600")

    stock_tree = ttk.Treeview(all_stock_window, columns=("stock",))
    stock_tree.heading("#0", text="품목")
    stock_tree.heading("stock", text="재고 수량")

    for item, stock in previous_total_stock.items():
        if stock < 100:
            stock_tree.insert("", "end", text=item, values=(stock,), tags=("red",))
        else:
            stock_tree.insert("", "end", text=item, values=(stock,))

    stock_tree.tag_configure("red", background="red", foreground="white")
    stock_tree.pack(expand=True, fill=tk.BOTH)

# GUI 생성
root = tk.Tk()
root.geometry("600x600")
root.title("재고관리 (채움플러스)")

frame = tk.Frame(root, bg="lightblue")
frame.pack(padx=20, pady=20, fill=tk.BOTH, expand=True)

search_label = tk.Label(frame, text="검색창:  품목명(한글자 이상) 입력 후 선택창을 클릭", bg="lightblue")
search_label.pack()
...
item_combobox = ttk.Combobox(frame, textvariable=item_var, state="readonly", justify="center", width=20, height=20)
item_combobox['values'] = items
item_combobox.pack(pady=15, fill=tk.X)

item_combobox.bind("<<ComboboxSelected>>", update_stock)
...
show_all_stock_button = tk.Button(frame, text="전체 재고 확인(100개 미만 빨강)", command=show_all_stock)
show_all_stock_button.pack(pady=10)
